 ...
 *
 */
static const float gyroVAR = 1e-7f;      // (rad/s)^2 / Hz
static const float biasVAR = 1e-8f;      // (rad/s)^2 / s (guessed)
 ...
 */
static const float MIN_VALID_CROSS_PRODUCT_MAG = 1.0e-3f;
...
status_t Fusion::handleMag(const vec3_t& m) {
	...
	/// !!!! FIXME: DIRTY HACK!!!
	/// Magnetic field projections are higher than allowed max length!!
    const float magFieldSq = length_squared(m) / 3;
